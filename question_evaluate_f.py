...
def evaluate_question(prompt):
    ...
    evaluation_prompt = f"""
    请根据以下标准评估问题：
    1. 问题质量：问题是否足够专业且符合能碳领域实际使用场景
    2. 上下文匹配：上下文信息语句是否足够完整且能否支撑问题的回答
...
if __name__ == "__main__":
    # 文件路径配置
    input_jsonl = "/workspace/DataProcess/bonito_output/carbon_questions_new_batch_1.jsonl"
    passed_jsonl = "/workspace/DataProcess/bonito_output/carbon_questions_new_batch_1_passed.jsonl"
    failed_jsonl = "/workspace/DataProcess/bonito_output/carbon_questions_new_batch_1_failed.jsonl"
    log_file = "/workspace/DataProcess/bonito_output/carbon_questions_new_batch_1_log.jsonl"
